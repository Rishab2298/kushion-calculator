  ...
  try {
        ...
        const categories = await prisma.fabricCategory.findMany({
          ...
          include: {
            ...
            fabrics: {
              ...
              take: 1,
              select: {
                id: true, name: true, imageUrl: true, pricePerSqInch: true,
                priceTier: true, discountEnabled: true, discountPercent: true,
                isDefault: true, brandId: true,
                brand: { select: { name: true } },
                patternAssignments: { select: { pattern: { select: { id: true, name: true } } } },
                colorAssignments: { select: { color: { select: { id: true, name: true, hexCode: true } } } },
                materialAssignments: { select: { material: { select: { id: true, name: true } } } },
              },
            },
          },
          orderBy: { sortOrder: "asc" },
    ...
    const responseData = JSON.stringify({
      ...
      fabricCategories: filteredFabricCategories.map(cat => ({
        ...
        defaultFabric: cat.defaultFabric ? {
          id: cat.defaultFabric.id,
          name: cat.defaultFabric.name,
          imageUrl: cat.defaultFabric.imageUrl,
          pricePerSqInch: cat.defaultFabric.pricePerSqInch,
          priceTier: cat.defaultFabric.priceTier,
          discountEnabled: cat.defaultFabric.discountEnabled || false,
          discountPercent: cat.defaultFabric.discountPercent || 0,
          isDefault: cat.defaultFabric.isDefault,
          categoryId: cat.id,
          categoryName: cat.name,
          brandId: cat.defaultFabric.brandId || null,
          brandName: cat.defaultFabric.brand?.name || null,
          patterns: (cat.defaultFabric.patternAssignments || []).map(a => ({ id: a.pattern.id, name: a.pattern.name })),
          colors: (cat.defaultFabric.colorAssignments || []).map(a => ({ id: a.color.id, name: a.color.name, hexCode: a.color.hexCode })),
          materialName: (cat.defaultFabric.materialAssignments || []).map(a => a.material.name).join(', ') || null,
          materials: (cat.defaultFabric.materialAssignments || []).map(a => ({ id: a.material.id, name: a.material.name })),
        } : null,
      })),
      ...
      settings: {
        ...
        flatRateThreshold: settings?.flatRateThreshold ?? 50,
        flatShippingAmount: settings?.flatShippingAmount ?? 50,
        flatLabourAmount: settings?.flatLabourAmount ?? 50,
