...
export const loader = async ({ request }) => {
  ...
  const antiSkidOptions = await prisma.antiSkidOption.findMany({
    where: { shop },
    orderBy: { sortOrder: "asc" },
  });

  return { antiSkidOptions, shop };
};
...
export const action = async ({ request }) => {
  ...
  if (intent === "create" || intent === "update") {
    ...
    if (data.isDefault) {
      await prisma.antiSkidOption.updateMany({
    ...
    if (intent === "create") {
      await prisma.antiSkidOption.create({ data });
    } else {
      await prisma.antiSkidOption.update({
  ...
  if (intent === "delete") {
    await prisma.antiSkidOption.delete({ where: { id: formData.get("id") } });
    return { success: true };
  }

  if (intent === "setDefault") {
    const id = formData.get("id");
    await prisma.antiSkidOption.updateMany({ where: { shop }, data: { isDefault: false } });
    await prisma.antiSkidOption.update({ where: { id }, data: { isDefault: true } });
    return { success: true };
  }
...
export default function AntiSkid() {
  const { antiSkidOptions } = useLoaderData();
  ...
  const resetForm = () => {
    setFormData({
      ...
      isActive: true,
      isDefault: false,
      sortOrder: antiSkidOptions.length,
    });
    setEditingItem(null);
  };
  ...
  const handleSubmit = () => {
    ...
    shopify.toast.show(editingItem ? "Anti-skid option updated" : "Anti-skid option created");
    setShowForm(false);
    resetForm();
  };

  const handleDelete = (id) => {
    if (confirm("Delete this anti-skid option?")) {
      const data = new FormData();
      data.append("intent", "delete");
      data.append("id", id);
      fetcher.submit(data, { method: "POST" });
      shopify.toast.show("Anti-skid option deleted");
    }
  };
  ...
  const handleSetDefault = (id) => {
    ...
    data.append("id", id);
    fetcher.submit(data, { method: "POST" });
    shopify.toast.show("Default anti-skid option updated");
  };

  return (
    <s-page heading="Anti-Skid Options" fullWidth>
      <s-button slot="primary-action" onClick={() => { resetForm(); setShowForm(true); }}>
        Add Anti-Skid Option
      </s-button>

      {showForm && (
        <s-section heading={editingItem ? "Edit Anti-Skid Option" : "Create Anti-Skid Option"}>
          <s-box padding="base" borderWidth="base" borderRadius="base">
            <s-stack direction="block" gap="base">
              <s-text-field
                ...
                placeholder="e.g., Standard Anti-Skid"
                required
              />
              ...
              <s-stack direction="inline" gap="base">
                ...
                />
                <label htmlFor="antiSkidActive">Active</label>
              </s-stack>
              ...
              <s-stack direction="inline" gap="base">
                ...
                />
                <label htmlFor="antiSkidDefault">Set as Default</label>
              </s-stack>
      ...
      <s-section heading={`Anti-Skid Options (${antiSkidOptions.length})`}>
        {antiSkidOptions.length === 0 ? (
          <s-box padding="base">
            <s-paragraph>No anti-skid options created yet.</s-paragraph>
          </s-box>
        ) : (
          <s-stack direction="block" gap="base">
            {antiSkidOptions.map((item) => (
              <s-box
                key={item.id}
                padding="base"
                borderWidth="base"
