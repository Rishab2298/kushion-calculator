...
CushionCalculator.prototype.openFabricBrowserPopup = function(categoryId, categoryName) {
  ...
  if (this.browserSelectedFabric) {
    ...
    document.getElementById('fabric-detail-pattern-' + blockId).textContent = this.browserSelectedFabric.patterns && this.browserSelectedFabric.patterns.length ? this.browserSelectedFabric.patterns.map(function(p) { return p.name; }).join(', ') : '-';
    document.getElementById('fabric-detail-color-' + blockId).textContent = this.browserSelectedFabric.colors && this.browserSelectedFabric.colors.length ? this.browserSelectedFabric.colors.map(function(c) { return c.name; }).join(', ') : '-';
...
CushionCalculator.prototype.selectPopupFabric = function(fabric) {
  ...
  document.getElementById('fabric-detail-pattern-' + blockId).textContent = fabric.patterns && fabric.patterns.length ? fabric.patterns.map(function(p) { return p.name; }).join(', ') : '-';
  document.getElementById('fabric-detail-color-' + blockId).textContent = fabric.colors && fabric.colors.length ? fabric.colors.map(function(c) { return c.name; }).join(', ') : '-';
